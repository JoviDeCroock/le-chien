...
- Strict rules for the \`\`\`preact block:
  ...
  - Plain JavaScript only. No TypeScript syntax: don't use type annotations, interfaces, \`as const\`, or \`value as SomeType\`.
  - No \`import\` or \`export\` statements. The server-side Dynamic Worker renderer injects globals: \`h\`, \`Fragment\`, \`useState\`, \`useEffect\`, \`useRef\`, \`useMemo\`, \`useCallback\`, and \`useInterval\`. Nothing else is available.
  ...
  - \`useEffect\` is supported for deterministic local state updates after render. Don't use it for browser APIs, network, storage, or long-running work.
  - For periodic updates, use the provided \`useInterval(callback, delayMs)\` at the top level of the component, for example \`useInterval(() => setCount((count) => count + 1), 30000)\`. Don't use \`setInterval\`, \`setTimeout\`, or \`requestAnimationFrame\`.
